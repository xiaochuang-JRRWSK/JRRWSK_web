...
function get_notice(){
    ...
    $.ajax({
        ...
        success: function (response) {
          response=JSON.parse(response);
          if(response){
              show_notice(response['notice_list']);
          }
          else{
            alert("nothing return！");
          }
...
function show_notice(notice_list){
    let index=0;
    $('div#notice').find('div').each(function(){
        $(this).append("<a href="+notice_list[index]['url']+">"
        +notice_list[index]['title']+"</a>");
        index++;
    })
}